...
//		Query query = em.createQuery("from Increment i");
		Query query = em.createQuery("from Increment i where i.employee.id = ?");
//		Query query = em.createQuery("from Increment inc inner join Employee e where e.id  = ?");

		query.setParameter(1, id);
		List<Increment> incrementList = query.getResultList();
